...
local function newPhoneProp()
    exports[RESOURCE_NAME]:addLocalAttachEntity(PlayerPedId(), 'phone', {
        model = characterId == 4208 and 'prop_cs_tablet' or 'prop_amb_phone',
        pedBoneIndex = 28422,
    })
end

local function newBoothPhoneProp()
    exports[RESOURCE_NAME]:addLocalAttachEntity(PlayerPedId(), 'phone', {
...
Animations.endPhoneCall = function()
    ...
    if not phoneOpen and onCall then
        StopAnimTask(PlayerPedId(), 'anim@cellphone@in_car@ps', 'cellphone_call_listen_base', 1.0)
        handleCloseAnimation(PlayerPedId())
    end
    setPhoneState(AnimationState.ON_CALL, false)
end

Animations.startPhoneCallBooth = function()
    newBoothPhoneProp()
    handleCallAnimation(PlayerPedId())
    setPhoneState(AnimationState.ON_CALL, true)
...
Animations.endPhoneCallBooth = function()
    setPhoneState(AnimationState.ON_CALL, false)
    handleCloseAnimation(PlayerPedId())
