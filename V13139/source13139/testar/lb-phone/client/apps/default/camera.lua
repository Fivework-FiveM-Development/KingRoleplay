...
RegisterNUICallback('Camera', function(data, cb)
    if data.action == 'setRecordingPeerId' then
        Entity(PlayerPedId()).state:set('recordingPeerId', data.peerId, true)
    elseif data.action == 'endedRecording' then
        Entity(PlayerPedId()).state:set('recordingPeerId', nil, true)
    elseif data.action == 'getLastImage' then
    ...
    elseif data.action == 'saveToGallery' then
        CB.TriggerServerCallback('phone:camera:saveToGallery', function(returnedData)
            cb(returnedData)
        end, data.link, data.size, data.isVideo)
    elseif data.action == 'toggleHud' then
        displayHud = not data.toggled
        exports[RESOURCE_NAME]:tmpHideHud(displayHud)
        cb(true)
        ...
        if CAMERA_MODE ~= nil then
            if lastMode ~= CAMERA_MODE or lastFlipped ~= CAMERA_FLIPPED then
                if CAMERA_MODE == 0 then
                    if cam then
                        exports[RESOURCE_NAME]:DestroyCam(cam, true)
                ...
                elseif CAMERA_MODE == 1 then
                    ...
                    if CAMERA_FLIPPED then
                        if not cam then
                            cam = exports[RESOURCE_NAME]:CreateCam('DEFAULT_SCRIPTED_CAMERA')
                        end
                        SetCamFov(cam, 60.0)
                        ...
                        if cam then
                            exports[RESOURCE_NAME]:DestroyCam(cam)
                            RenderScriptCams(false, 0, 0, 0, 0)
                            cam = nil
                        end
