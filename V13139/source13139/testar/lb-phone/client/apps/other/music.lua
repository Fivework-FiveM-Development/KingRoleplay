...
RegisterNUICallback('Music', function(data, cb)
    ...
    elseif data.action == 'getConfig' then
        AVA.TriggerServerCallback('ava:music:getConfig', function(data)
            cb(data)
        end, data)
    ...
    elseif data.action == 'viewed' then
        TriggerServerEvent('ava:music:viewed', data.songId)
    elseif data.action == 'like' then
        cachedLikes[data.songId] = data.like
        TriggerServerEvent('ava:music:like', data.songId, data.like)
