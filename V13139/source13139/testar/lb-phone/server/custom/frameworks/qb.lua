...
Citizen.Trace("Loading QB")
QB = exports["qb-core"]:GetCoreObject()
Citizen.Trace("QB loaded")
...
Citizen.Trace("using old method to refresh companies")
...
Citizen.Trace("qb jobs: initial data", playerJobs, jobCounts, jobDutyCounts)
end)
...
Citizen.Trace("qb jobs: player loaded update (src, job, duty)", src, job.name, job.onduty)
end)
...
Wait(0)
Citizen.Trace("qb jobs: job update (src, job, duty)", src, job.name, job.onduty)
...
Citizen.Trace(playerJobs, jobCounts, jobDutyCounts)
end)
...
Wait(0)
Citizen.Trace("qb jobs: player unload", src)
...
Citizen.Trace("qb jobs: player dropped", src)
UnloadJob(src)
end)

ESX.RegisterServerCallback("services:getPlayerData", function(_, cb, player)
...
ESX.RegisterServerCallback("services:getAccount", function(source, cb)
local job = GetJob(source)

cb(GetSocietyMoney(job))
end)

ESX.RegisterServerCallback("services:addMoney", function(source, cb, amount)
local job = GetJob(source)
...
ESX.RegisterServerCallback("services:removeMoney", function(source, cb, amount)
local job = GetJob(source)
...
ESX.RegisterServerCallback("crypto:getOtherQBitWallet", function(source, cb, otherNumber)
