...
if IsDuplicityVersion() then-- SERVER CALLBACKS
    ...
    local TriggerServerCallback = function(name, source, cb, ...)
        local phoneNumber = exports[RESOURCE_NAME]:getPhoneNumber(source)
    ...
    RegisterNetEvent('phone:cb:triggerServerCallback', function(name, requestId, ...)
        local source = source
        TriggerServerCallback(name, source, function(...)
            TriggerClientEvent('phone:cb:serverCallback:' .. name .. ':' .. requestId, source, ...)
        end, ...)
    end)
else
    ...
    CB.TriggerServerCallback = function(name, cb, ...)
        TriggerServerEvent('phone:cb:triggerServerCallback', name, ServerCBCurrentRequestId, ...)

        local eventHandler = nil
        local timeout = nil

        eventHandler = RegisterNetEvent('phone:cb:serverCallback:' .. name .. ':' .. ServerCBCurrentRequestId, function(...)
